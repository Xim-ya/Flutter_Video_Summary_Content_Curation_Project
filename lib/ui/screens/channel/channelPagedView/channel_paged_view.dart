...
import 'package:movie_curation/ui/screens/channel/contentDetail/channel_content_detail_screen.dart';
...
class ChannelPagedView extends HookWidget {
  const ChannelPagedView({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    //  PagedViewScreen의 Screen Index, 0 : 채널 스크린, 1 : 컨텐츠 상세 스크린
    ...
    void _pagedRouteHandler() {
      if (Get.isRegistered<ChannelContentDetailViewModel>()) {
        // 직접 Controller을 삭제.
        Get.delete<ChannelContentDetailViewModel>();
      } else {
        // `ContentDetail` VM Controller 생성
        Get.lazyPut(() =>
            ChannelContentDetailViewModel(Get.find(), Get.find(), Get.find()));
      }
      // PagedView 라우팅
    ...
    final screenList = [
      ChannelScreen(routeAction: _pagedRouteHandler),
      _screenIndex.value == 1
          ? ChannelContentDetailScreen(routeAction: _pagedRouteHandler)
          : const SizedBox(),
    ];
